namespace :import_usda_csv_files do
  ...
  task :perform, [:num] => [:environment] do |t, args|
    puts "Perform Step #{args[:num].inspect}"
    report, errors = ImportUsdaCsvFiles.perform(args[:num].to_i)
    puts ""
    puts "Import Csv Files Report (line count: #{report.count})"
    report.each do |line|
      puts line
    end
    puts ""
    puts "perform1 Import Csv Files Errors (errors count: #{errors.count}):"
